...
class Pesapal
{
    ...
    function checkPaymentStatus($pesapal_merchant_reference,$pesapalTrackingId) {
      $consumer_key = config('pesapal.consumer_key');

      $consumer_secret = config('pesapal.consumer_secret');

      $statusrequestAPI = config('pesapal.live') ? "https://www.pesapal.com/api/querypaymentstatus" : "https://demo.pesapal.com/api/querypaymentstatus";
       $token = $params = NULL;
       $consumer = new OAuthConsumer($consumer_key, $consumer_secret);
       $signature_method = new OAuthSignatureMethod_HMAC_SHA1();
